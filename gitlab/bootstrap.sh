...
check_ports() {
    ...
    if netstat -tuln 2>/dev/null | grep -q ":80 " || ss -tuln 2>/dev/null | grep -q ":80 "; then
        log_warn "端口 80 (GitLab HTTP) 已被占用"
    fi

    if netstat -tuln 2>/dev/null | grep -q ":443 " || ss -tuln 2>/dev/null | grep -q ":443 "; then
        log_warn "端口 443 (GitLab HTTPS) 已被占用"
    fi

    if netstat -tuln 2>/dev/null | grep -q ":22 " || ss -tuln 2>/dev/null | grep -q ":22 "; then
        log_warn "端口 22 (GitLab SSH) 已被占用"
    fi
...
init_only() {
    ...
    echo ""
    echo "🌐 服务端口："
    echo "- GitLab HTTP: http://localhost"
    echo "- GitLab HTTPS: https://localhost"
    echo "- GitLab SSH: localhost:22"
    echo ""
}
