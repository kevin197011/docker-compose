...
check_ports() {
    ...
    # 检查 ELK Stack 端口 9200
    if netstat -tuln 2>/dev/null | grep -q ":9200 " || ss -tuln 2>/dev/null | grep -q ":9200 "; then
        log_warn "端口 9200 (ELK Stack) 已被占用"
    fi

    # 检查 ELK Stack 端口 5601
    if netstat -tuln 2>/dev/null | grep -q ":5601 " || ss -tuln 2>/dev/null | grep -q ":5601 "; then
        log_warn "端口 5601 (ELK Stack) 已被占用"
    fi
...
init_only() {
    ...
    log_success "ELK Stack 项目环境初始化完成！"
    echo ""
    echo "🚀 接下来的步骤："
    ...
    echo ""
    echo "🌐 服务端口："
    echo "- ELK Stack: http://localhost:9200"
    echo "- ELK Stack: http://localhost:5601"
    echo ""
}
