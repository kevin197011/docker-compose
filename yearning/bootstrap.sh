...
check_ports() {
    ...
    if netstat -tuln 2>/dev/null | grep -q ":8000 " || ss -tuln 2>/dev/null | grep -q ":8000 "; then
        log_warn "端口 8000 (Yearning) 已被占用"
    fi
...
init_only() {
    ...
    log_success "Yearning 项目环境初始化完成！"
    echo ""
    echo "🚀 接下来的步骤："
    ...
    echo ""
    echo "🌐 服务端口："
    echo "- Yearning: http://localhost:8000"
    echo ""
}
