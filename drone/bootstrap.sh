...
check_ports() {
    ...
    if netstat -tuln 2>/dev/null | grep -q ":3001 " || ss -tuln 2>/dev/null | grep -q ":3001 "; then
        log_warn "端口 3001 (Drone CI) 已被占用"
    fi
...
init_only() {
    ...
    log_success "Drone CI 项目环境初始化完成！"
    echo ""
    echo "🚀 接下来的步骤："
    ...
    echo ""
    echo "🌐 服务端口："
    echo "- Drone CI: http://localhost:3001"
    echo ""
}
