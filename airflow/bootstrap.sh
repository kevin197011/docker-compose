...
check_ports() {
    ...
    # 检查 Apache Airflow 端口 8080
    if netstat -tuln 2>/dev/null | grep -q ":8080 " || ss -tuln 2>/dev/null | grep -q ":8080 "; then
        log_warn "端口 8080 (Apache Airflow) 已被占用"
    fi
...
init_only() {
    ...
    log_success "Apache Airflow 项目环境初始化完成！"
    echo ""
    echo "🚀 接下来的步骤："
...
main() {
    ...
    log_info "开始 Apache Airflow 一体化部署..."
    ...
    # 显示访问信息
    echo ""
    log_success "Apache Airflow 部署完成！"
    echo ""
    echo "🌐 访问地址:"
    echo "  Apache Airflow: http://localhost:8080"
    echo ""
