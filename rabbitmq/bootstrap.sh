...
check_ports() {
    ...
    if netstat -tuln 2>/dev/null | grep -q ":5672 " || ss -tuln 2>/dev/null | grep -q ":5672 "; then
        log_warn "端口 5672 (RabbitMQ) 已被占用"
    fi

    # 检查 RabbitMQ 端口 15672
    if netstat -tuln 2>/dev/null | grep -q ":15672 " || ss -tuln 2>/dev/null | grep -q ":15672 "; then
        log_warn "端口 15672 (RabbitMQ) 已被占用"
    fi
...
init_only() {
    ...
    log_success "RabbitMQ 项目环境初始化完成！"
    echo ""
    echo "🚀 接下来的步骤："
    ...
    echo ""
    echo "🌐 服务端口："
    echo "- RabbitMQ: http://localhost:5672"
