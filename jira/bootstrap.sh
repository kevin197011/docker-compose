...
check_ports() {
    ...
    # 检查 Atlassian JIRA 端口 8080
    if netstat -tuln 2>/dev/null | grep -q ":8080 " || ss -tuln 2>/dev/null | grep -q ":8080 "; then
        log_warn "端口 8080 (Atlassian JIRA) 已被占用"
    fi
...
init_only() {
    ...
    log_success "Atlassian JIRA 项目环境初始化完成！"
    echo ""
    echo "🚀 接下来的步骤："
    ...
    echo ""
    echo "🌐 服务端口："
    echo "- Atlassian JIRA: http://localhost:8080"
    echo ""
}
...
main() {
    ...
    log_info "开始 Atlassian JIRA 一体化部署..."
    ...
    # 显示访问信息
    echo ""
    log_success "Atlassian JIRA 部署完成！"
    echo ""
    echo "🌐 访问地址:"
    echo "  Atlassian JIRA: http://localhost:8080"
    echo ""
