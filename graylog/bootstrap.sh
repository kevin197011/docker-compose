...
check_ports() {
    ...
    if netstat -tuln 2>/dev/null | grep -q ":9000 " || ss -tuln 2>/dev/null | grep -q ":9000 "; then
        log_warn "端口 9000 (Graylog) 已被占用"
    fi

    # 检查 Graylog 端口 12201
    if netstat -tuln 2>/dev/null | grep -q ":12201 " || ss -tuln 2>/dev/null | grep -q ":12201 "; then
        log_warn "端口 12201 (Graylog) 已被占用"
    fi
...
init_only() {
    ...
    echo "- Graylog: http://localhost:12201"
    echo ""
}
