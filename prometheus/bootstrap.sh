...
check_ports() {
    ...
    # 检查 Prometheus Stack 端口 9090
    if netstat -tuln 2>/dev/null | grep -q ":9090 " || ss -tuln 2>/dev/null | grep -q ":9090 "; then
        log_warn "端口 9090 (Prometheus Stack) 已被占用"
    fi

    # 检查 Prometheus Stack 端口 3000
    if netstat -tuln 2>/dev/null | grep -q ":3000 " || ss -tuln 2>/dev/null | grep -q ":3000 "; then
        log_warn "端口 3000 (Prometheus Stack) 已被占用"
    fi
...
init_only() {
    ...
    log_success "Prometheus Stack 项目环境初始化完成！"
    echo ""
    echo "🚀 接下来的步骤："
    ...
    echo ""
    echo "🌐 服务端口："
    echo "- Prometheus Stack: http://localhost:9090"
    echo "- Prometheus Stack: http://localhost:3000"
    echo ""
}
...
main() {
    ...
    log_info "开始 Prometheus Stack 一体化部署..."
    ...
    # 显示访问信息
    echo ""
    log_success "Prometheus Stack 部署完成！"
    echo ""
    echo "🌐 访问地址:"
    echo "  Prometheus Stack: http://localhost:9090"
    echo "  Prometheus Stack: http://localhost:3000"
