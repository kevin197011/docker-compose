...
check_ports() {
    ...
    # 检查 Apache APISIX 端口 9080
    if netstat -tuln 2>/dev/null | grep -q ":9080 " || ss -tuln 2>/dev/null | grep -q ":9080 "; then
        log_warn "端口 9080 (Apache APISIX) 已被占用"
    fi

    # 检查 Apache APISIX 端口 9443
    if netstat -tuln 2>/dev/null | grep -q ":9443 " || ss -tuln 2>/dev/null | grep -q ":9443 "; then
        log_warn "端口 9443 (Apache APISIX) 已被占用"
    fi
...
init_only() {
    ...
    log_success "Apache APISIX 项目环境初始化完成！"
    echo ""
    echo "🚀 接下来的步骤："
    ...
    echo "- Apache APISIX: http://localhost:9443"
    echo ""
}
...
main() {
    ...
    log_info "开始 Apache APISIX 一体化部署..."
    ...
    # 显示访问信息
    echo ""
    log_success "Apache APISIX 部署完成！"
    echo ""
    echo "🌐 访问地址:"
    echo "  Apache APISIX: http://localhost:9080"
    echo "  Apache APISIX: http://localhost:9443"
