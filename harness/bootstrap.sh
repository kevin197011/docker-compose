...
check_ports() {
    ...
    # 检查 Harness 端口 7143
    if netstat -tuln 2>/dev/null | grep -q ":7143 " || ss -tuln 2>/dev/null | grep -q ":7143 "; then
        log_warn "端口 7143 (Harness) 已被占用"
    fi

    # 检查 Harness 端口 8080
    if netstat -tuln 2>/dev/null | grep -q ":8080 " || ss -tuln 2>/dev/null | grep -q ":8080 "; then
        log_warn "端口 8080 (Harness) 已被占用"
    fi
...
init_only() {
    ...
    echo ""
    echo "🌐 服务端口："
    echo "- Harness: http://localhost:7143"
