...
if [ -z "$IP_ADDRESS" ]; then
    ...
    echo "Please set it in .env file: IP_ADDRESS=your_server_ip"
    exit 1
fi
...
if [ -z "$DRONE_RPC_SECRET" ]; then
    ...
    print_color $YELLOW "Please add this to your .env file: DRONE_RPC_SECRET=$DRONE_RPC_SECRET"
fi
...
# Note: Nginx configuration is now handled by nginx templating system
print_color $YELLOW "Nginx templates are ready - will be processed automatically by nginx container"
print_color $GREEN "✓ Using nginx template system for configuration"
