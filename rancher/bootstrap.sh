...
check_ports() {
    ...
    if netstat -tuln 2>/dev/null | grep -q ":80 " || ss -tuln 2>/dev/null | grep -q ":80 "; then
        log_warn "端口 80 (Rancher) 已被占用"
    fi

    # 检查 Rancher 端口 443
    if netstat -tuln 2>/dev/null | grep -q ":443 " || ss -tuln 2>/dev/null | grep -q ":443 "; then
        log_warn "端口 443 (Rancher) 已被占用"
    fi
...
init_only() {
    ...
    echo ""
    echo "🌐 服务端口："
    echo "- Rancher: http://localhost:80"
    echo "- Rancher: http://localhost:443"
    echo ""
}
