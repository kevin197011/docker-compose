...
[[ -f "$AGENT_HOST" ]] || { echo "[ERROR] missing $AGENT_HOST"; exit 1; }
...
read_server_id_from_cfg() {
  docker exec "$CONTAINER" sh -c '
    for f in \
      ...
      /var/atlassian/application-data/confluence/shared-home/confluence.cfg.xml; do
      [ -f "$f" ] || continue
...
detect_server_id() {
  ...
  echo "[ERROR] Server ID not found; open Confluence setup page first or set SERVER_ID" >&2
  return 1
}
