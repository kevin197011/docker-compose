...
check_ports() {
    ...
    # 检查 Gaia Pipeline 端口 8080
    if netstat -tuln 2>/dev/null | grep -q ":8080 " || ss -tuln 2>/dev/null | grep -q ":8080 "; then
        log_warn "端口 8080 (Gaia Pipeline) 已被占用"
    fi
...
init_only() {
    ...
    log_success "Gaia Pipeline 项目环境初始化完成！"
    echo ""
    echo "🚀 接下来的步骤："
    ...
    echo ""
    echo "🌐 服务端口："
    echo "- Gaia Pipeline: http://localhost:8080"
    echo ""
}
...
main() {
    ...
    log_info "开始 Gaia Pipeline 一体化部署..."
    ...
    # 显示访问信息
    echo ""
    log_success "Gaia Pipeline 部署完成！"
    echo ""
    echo "🌐 访问地址:"
    echo "  Gaia Pipeline: http://localhost:8080"
    echo ""
