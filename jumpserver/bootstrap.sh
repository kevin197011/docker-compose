...
check_ports() {
    ...
    if netstat -tuln 2>/dev/null | grep -q ":80 " || ss -tuln 2>/dev/null | grep -q ":80 "; then
        log_warn "端口 80 (JumpServer) 已被占用"
    fi

    # 检查 JumpServer 端口 443
    if netstat -tuln 2>/dev/null | grep -q ":443 " || ss -tuln 2>/dev/null | grep -q ":443 "; then
        log_warn "端口 443 (JumpServer) 已被占用"
    fi

    # 检查 JumpServer 端口 2222
    if netstat -tuln 2>/dev/null | grep -q ":2222 " || ss -tuln 2>/dev/null | grep -q ":2222 "; then
        log_warn "端口 2222 (JumpServer) 已被占用"
    fi
...
init_only() {
    ...
    log_success "JumpServer 项目环境初始化完成！"
    echo ""
    echo "🚀 接下来的步骤："
    ...
    echo ""
    echo "🌐 服务端口："
    echo "- JumpServer: http://localhost:80"
    echo "- JumpServer: http://localhost:443"
    echo "- JumpServer: http://localhost:2222"
    echo ""
}
